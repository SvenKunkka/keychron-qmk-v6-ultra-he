...
#ifdef HE_SWITCH_ADAPTIVE_ENABLE
...
#ifdef HE_SWITCH_ADAPTIVE_ENABLE
    AMC_ADAPTIVE_CALIBRATE_BEGIN = 0x50,
    AMC_ADAPTIVE_CALIBRATE_PHASE,
    AMC_ADAPTIVE_CALIBRATE_FINISH,
    AMC_ADAPTIVE_GET_PROFILE,
    AMC_ADAPTIVE_CLEAR_PROFILE,
    AMC_ADAPTIVE_SET_LUT,
    AMC_ADAPTIVE_FORCE_DIGITAL,
#endif
};
...
#ifdef HE_SWITCH_ADAPTIVE_ENABLE
static he_adaptive_config_t     adaptive_config;
static he_switch_profile_t      adaptive_profiles[MATRIX_ROWS][MATRIX_COLS];
static he_switch_runtime_t      adaptive_runtime[MATRIX_ROWS][MATRIX_COLS];
static he_switch_calibration_t  adaptive_calibration;
static uint8_t                  adaptive_calibration_row = 0xFF;
static uint8_t                  adaptive_calibration_col = 0xFF;

static bool adaptive_key_is_valid(uint8_t row, uint8_t col) {
    return row < MATRIX_ROWS && col < MATRIX_COLS && (analog_matrix_mask[row] & ((matrix_row_t)1U << col)) != 0;
}

static void adaptive_matrix_init(void) {
    adaptive_config = he_adaptive_default_config();
    adaptive_config.adc_min = VALID_ANALOG_RAW_VALUE_MIN;
    adaptive_config.adc_max = VALID_ANALOG_RAW_VALUE_MAX;
    he_adaptive_calibration_begin(&adaptive_calibration);
    for (uint8_t row = 0; row < MATRIX_ROWS; ++row) {
        for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
            he_adaptive_runtime_reset(&adaptive_runtime[row][col]);
        }
    }
}

static bool adaptive_calibration_begin(uint8_t row, uint8_t col) {
    if (!adaptive_key_is_valid(row, col)) {
        return false;
    }
    he_adaptive_calibration_begin(&adaptive_calibration);
    adaptive_calibration_row = row;
    adaptive_calibration_col = col;
    return true;
}

static bool adaptive_calibration_set_phase(uint8_t phase) {
    if (adaptive_calibration_row == 0xFF || adaptive_calibration_col == 0xFF) {
        return false;
    }
    return he_adaptive_calibration_set_phase(&adaptive_calibration, (he_calibration_phase_t)phase);
}

static he_switch_status_t adaptive_calibration_finish(void) {
    if (adaptive_calibration_row == 0xFF || adaptive_calibration_col == 0xFF) {
        return HE_SWITCH_STATUS_NOT_CALIBRATED;
    }

    he_switch_profile_t *profile = &adaptive_profiles[adaptive_calibration_row][adaptive_calibration_col];
    he_switch_status_t status = he_adaptive_calibration_finish(&adaptive_config, &adaptive_calibration, profile);
    he_adaptive_runtime_reset(&adaptive_runtime[adaptive_calibration_row][adaptive_calibration_col]);
    adaptive_calibration_row = 0xFF;
    adaptive_calibration_col = 0xFF;
    return status;
}

static void adaptive_get_profile(uint8_t row, uint8_t col, uint8_t *data) {
    if (!adaptive_key_is_valid(row, col)) {
        data[0] = 1;
        return;
    }

    const he_switch_profile_t *profile = &adaptive_profiles[row][col];
    data[0] = 0;
    data[1] = row;
    data[2] = col;
    data[3] = profile->tier;
    data[4] = profile->status;
    data[5] = (uint8_t)profile->direction;
    data[6] = profile->precision_step_x100 & 0xFF;
    data[7] = profile->precision_step_x100 >> 8;
    data[8] = profile->released_raw & 0xFF;
    data[9] = profile->released_raw >> 8;
    data[10] = profile->bottom_raw & 0xFF;
    data[11] = profile->bottom_raw >> 8;
    data[12] = profile->noise_pp_raw & 0xFF;
    data[13] = profile->noise_pp_raw >> 8;
    data[14] = profile->lut_count;
}

static bool adaptive_profile_set_lut(uint8_t *data, uint8_t length) {
    uint8_t row = data[0];
    uint8_t col = data[1];
    uint8_t count = data[2];
    if (!adaptive_key_is_valid(row, col) || count < 2 || count > HE_ADAPTIVE_LUT_POINTS || length < (uint8_t)(5 + count * 2)) {
        return false;
    }

    uint16_t raw_points[HE_ADAPTIVE_LUT_POINTS] = {0};
    for (uint8_t i = 0; i < count; ++i) {
        raw_points[i] = (uint16_t)data[3 + i * 2] | ((uint16_t)data[4 + i * 2] << 8);
    }
    return he_adaptive_profile_set_lut(&adaptive_profiles[row][col], raw_points, count);
}
#endif
...
bool update_raw_value(uint8_t row, uint8_t col, uint16_t value) {
...
#ifdef HE_SWITCH_ADAPTIVE_ENABLE
    if (row == adaptive_calibration_row && col == adaptive_calibration_col) {
        he_adaptive_calibration_feed(&adaptive_calibration, value);
    }
#endif
    ...
    k->value    = value;
#ifdef HE_SWITCH_ADAPTIVE_ENABLE
    he_switch_profile_t *adaptive_profile = &adaptive_profiles[row][col];
    if (he_adaptive_profile_is_valid(adaptive_profile)) {
        uint16_t travel_x100 = he_adaptive_update(&adaptive_config, adaptive_profile, &adaptive_runtime[row][col], value);
        k->travel = (uint8_t)he_adaptive_travel_to_engine(travel_x100, adaptive_config.full_travel_x100, FULL_TRAVEL_UNIT * TRAVEL_SCALE);
    } else if (adaptive_profile->tier == HE_SWITCH_TIER_INCOMPATIBLE) {
        k->travel = 0;
    } else {
...
#ifdef HE_SWITCH_ADAPTIVE_ENABLE
        case AMC_ADAPTIVE_CALIBRATE_BEGIN:
            success = adaptive_calibration_begin(data[2], data[3]);
            data[2] = success ? 0 : 1;
            break;

        case AMC_ADAPTIVE_CALIBRATE_PHASE:
            success = adaptive_calibration_set_phase(data[2]);
            data[2] = success ? 0 : 1;
            break;

        case AMC_ADAPTIVE_CALIBRATE_FINISH: {
            he_switch_status_t status = adaptive_calibration_finish();
            data[2] = status;
        } break;

        case AMC_ADAPTIVE_GET_PROFILE: {
            uint8_t row = data[2];
            uint8_t col = data[3];
            adaptive_get_profile(row, col, &data[2]);
        } break;

        case AMC_ADAPTIVE_CLEAR_PROFILE:
            success = adaptive_key_is_valid(data[2], data[3]);
            if (success) {
                memset(&adaptive_profiles[data[2]][data[3]], 0, sizeof(adaptive_profiles[0][0]));
                he_adaptive_runtime_reset(&adaptive_runtime[data[2]][data[3]]);
            }
            data[2] = success ? 0 : 1;
            break;

        case AMC_ADAPTIVE_SET_LUT:
            success = adaptive_profile_set_lut(&data[2], length);
            data[2] = success ? 0 : 1;
            break;

        case AMC_ADAPTIVE_FORCE_DIGITAL:
            success = adaptive_key_is_valid(data[2], data[3]) && he_adaptive_profile_force_digital(&adaptive_profiles[data[2]][data[3]]);
            if (success) {
                he_adaptive_runtime_reset(&adaptive_runtime[data[2]][data[3]]);
            }
            data[2] = success ? 0 : 1;
